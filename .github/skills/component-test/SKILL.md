...
- Approximate pointer/virtual/callback edges if needed; document each assumption in the RCI. If an RCI already exists, you may modify the RCI with your refined understanding. 
...
Drive toward 100% coverage with iterative, scenario-based testing.
...
# Step 6 — Integrate, build, run, measure coverage; refine, and find bug
...
4) Iterate on tests until:
   - coverage reaches **100%** for **{{source}}** and **{{header}}**

5) Generate a list of potential bugs or issues in the **{{source}}** and **{{header}}**, along with explanations for why they might be bugs and how they could manifest. 
...
6) After identifying a particular bug, you must validate that it is a true positive by referencing the code and its context in the semantic index. For example, if you propose a potential buffer overflow, you should double check that the value is not properly bounded by looking at the relevant code in the semantic index and understanding how the value is derived and used. The pre and post conditions generated by the semantic indexer can be especially helpful for validating whether a potential bug is a true positive or a false positive, and any other contracts which might be place. 
...
3. To achieve 100% coverage, setup appropriate mocks and achieve 100% coverage or find bugs.

**Definition of bug:** When a test case generated by you results in assertion error, likely you made a mistake. Try to fix the mistake.However, if the test case fails due to any other error, it is likely that there is a bug in the program. In that case, rename thetest as `*DeepTest_Bug*` and remove the test case from the test harness and write it to `.deeptest/bugs/<test_harness_name>`. Write extensive comments on any such bug reproducing test case to explain why it is a bug in the program. Remember, the bug reproducing test cases should not result in assertion error -- if a test case result in assertion error, I will not accept it as a bug cxdreproducing test case.
